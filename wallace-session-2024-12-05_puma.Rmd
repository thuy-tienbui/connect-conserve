...
occs_path <- ""
occs_path <- file.path(occs_path, "puma_concolor2.csv")
# get a list of species occurrence data
userOccs_Pc <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "puma_concolor2.csv", 
  txtSep = ",", 
  txtDec = ".")
occs_Pc <- userOccs_Pc$Puma_concolor$cleaned
```
...
dir_envs_Pc <- ""
envs_path <- file.path(dir_envs_Pc, c('cal_veg_270m_wgs84_NEW_Clip.tif', 'dem_270m_wgs84_Clip.tif', 'distance_water_270m_wgs84_Clip.tif', 'mean_ppt_270m_wgs84_Clip.tif', 'mean_temp_270m_wgs84_Clip.tif', 'season_ppt_270m_wgs84_Clip.tif', 'season_temp_270m_wgs84_Clip.tif'))
# Create environmental object 
envs_Pc <- envs_userEnvs(
  rasPath = envs_path,
  ...
  doBrick = FALSE)
occs_xy_Pc <- occs_Pc[c('longitude', 'latitude')]
occs_vals_Pc <- as.data.frame(raster::extract(envs_Pc, occs_xy_Pc, cellnumbers = TRUE))
# Remove duplicated same cell values
occs_Pc <- occs_Pc[!duplicated(occs_vals_Pc[, 1]), ]
occs_vals_Pc <- occs_vals_Pc[!duplicated(occs_vals_Pc[, 1]), -1]
...
Thinning the occurrences to 12 km

```{r}
# Thin occurrences 
occs_Pc <- poccs_thinOccs(
  occs = occs_Pc, 
  thinDist = 12)
```
...
bgEnvsVals_Pc <- cbind(scientific_name = paste0("bg_", "Puma concolor"), bgSample_Pc,
                            occID = NA, year = NA, institution_code = NA, country = NA,
                            state_province = NA, locality = NA, elevation = NA,
                            record_type = NA, bgEnvsVals_Pc)
```
...
m_Pc <- model_Pc@models[["fc.LQ_rm.0.5"]]
predSel_Pc <- predictMaxnet(m_Pc, bgMask_Pc,
                                          type = "cloglog", 
                                          clamp = TRUE)
#Get values of prediction
mapPredVals_Pc <- getRasterVals(predSel_Pc, "cloglog")
#Define colors and legend  
rasCols <- c("#2c7bb6", "#abd9e9", "#ffffbf", "#fdae61", "#d7191c")
legendPal <- colorNumeric(rev(rasCols), mapPredVals_Pc, na.color = 'transparent')
rasPal <- colorNumeric(rasCols, mapPredVals_Pc, na.color = 'transparent')
#Generate map
m <- leaflet() %>% addProviderTiles(providers$Esri.WorldTopoMap) 
m  %>%
  leaflet::addLegend("bottomright", pal = legendPal,
            title = "Predicted Suitability<br>(Training)",
            values = mapPredVals_Pc, layerId = "train",
            labFormat = reverseLabel(2, reverse_order = TRUE)) %>% 
  #add occurrence data
  addCircleMarkers(data = occs_Pc, lat = ~latitude, lng = ~longitude,
                   radius = 5, color = 'red', fill = TRUE, fillColor = "red",
                   fillOpacity = 0.2, weight = 2, popup = ~pop) %>% 
  ##Add model prediction
  addRasterImage(predSel_Pc, colors = rasPal, opacity = 0.7,
                 group = 'vis', layerId = 'mapPred', method = "ngb") %>%
 ##add background polygons
  addPolygons(data = bgExt_Pc,fill = FALSE,
...
n <- mxNonzeroCoefs(model_Pc@models[["fc.LQ_rm.0.5"]], "maxnet")

# Create response curves
for (i in n) {
maxnet::response.plot(
  model_Pc@models[["fc.LQ_rm.0.5"]],
  v = i,
  type = "cloglog")
}
```
